...
Catalog repo publish workflows generated by `gx catalog init` now publish both:
- the canonical root catalog under `ghcr.io/<owner>/catalogs/<repo>/catalog.json:latest`
- a tar bundle under `ghcr.io/<owner>/<repo>-bundle:latest`

Downstream catalogs can keep direct catalog consumption for discovery and use
bundle references in entry metadata when template assets should be materialized
from an extracted bundle instead of fetched as standalone OCI JSON files.
